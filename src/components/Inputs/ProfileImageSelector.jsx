...
function ProfileImageSelector(props) {
    const { image,setImage, preview, setPreview } = props
    ...
    const handleImageChange = async (e) => {
        ...
        if(file) {
            ...
            // generate the preview image url from file
    ...
    return (
        <div className="flex justify-center mb-6">
            <input type="file" className="hidden" accept={"image/*"} ref={inputRef} onChange={handleImageChange} />
            {
                !image ? (
                    ...
                    <div className="relative">
                        ...
                        />
                        <button className="w-8 h-8 flex items-center justify-center bg-red-500 text-white rounded-full absolute -bottom-1 -right-1 cursor-pointer" onClick={handleRemoveImage}>
                            <LuTrash/>
                        </button>
                    </div>
                )
