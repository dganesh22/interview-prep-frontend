            ...
            <div className="text-[14px] prose prose-slate dark:prose-invert max-w-none">
                <ReactMarkdown
                    ...
                    components={{
                        ...
                        h5({children}) {
                            return <h5 className="text-md font-bold mt-6 mb-3">{children}</h5>;
                        },
                        h6({children}) {
                            return <h6 className="text-sm font-bold mt-8 mt-4">{children}</h6>;
                        },
                        a({children, href}) {
                        ...
                        code({children, node, className, ...props }) {
                            ...
                            return !isInline ? (
                                ...
                                <code className={"text-sm font-bold text-gray-700 italic"} {...props}> {children} </code>
                            );
                        }
                    }}
                >
...
function CodeBlock({ code, language }) {
    ...
    return (
        <div className={"relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-300"}>
            <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200">
                <div className="flex items-center space-x-2">
                    <LuCode size={16} className={"text-gray-700"} />
                    <span className="text-xs font-semibold text-gray-900 uppercase tracking-wide">
                        { language || "Code"}
                    </span>
                </div>
                <button onClick={copyCode} className="text-gray-500 hover:text-gray-700 focus:outline-none relative cursor-pointer group">
                    { copied ? (
                        <LuCheck size={16} className={"text-green-600"} />
                    ): (
                        <LuCopy size={16} className={"text-dark-600"} />
                    )}
                    { copied ? (
                        <span className={"absolute  top-0 right-8 bg-black text-white text-xs rounded-md px-2 py-1 opacity-80 group-hover:opacity-100 transition"}> Copied!.. </span>
                    ): ""}
                </button>
            </div>
